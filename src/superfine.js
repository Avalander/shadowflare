...
var createNode = function(vnode, isSvg) {
  ...
  for (var i = 0, len = vnode.children.length; i < len; i++) {
    var elem = createNode(vnode.children[i], isSvg)
    node.appendChild(elem)
  }

  if (vnode.props.oncreate) {
    vnode.props.oncreate(node)
  }
...
var removeNode = function (parent, vnode) {
  if (vnode.props.ondestroy) {
    vnode.props.ondestroy(vnode.node)
  }
  destroyChildren(vnode)
  // for (var i = 0; i < vnode.children.length; i++) {
  //   removeNode(vnode.node, vnode.children[i])
  // }
  parent.removeChild(vnode.node)
}

var destroyChildren = function (vnode) {
  for (var i = 0; i < vnode.children.length; i++) {
    destroyChildren(vnode.children[i])
    if (vnode.children[i].props.ondestroy) {
      vnode.children[i].props.ondestroy(vnode.children[i].node)
    }
  }
}
